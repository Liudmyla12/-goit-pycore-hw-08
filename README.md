...
Бот підтримує роботу з контактами, телефонами, днями народження та показує найближчі дні народження.
...
Файл даних addressbook.pkl створюється автоматично та ігнорується Git’ом.
...
При наступному запуску — будуть відновлені.
...
change [ім'я] [старий] [новий] — замінити номер

phone [ім'я] — показати телефони контакту

all — показати всі контакти

add-birthday [ім'я] [ДД.ММ.РРРР] — додати/оновити дату народження
...
Телефон — 10 цифр (інше → Error: Phone must contain exactly 10 digits)

День народження — DD.MM.YYYY (інше → Error: Invalid date format. Use DD.MM.YYYY)
...
Варіант А — демонстраційний (щоб birthdays точно щось показав):
